-- Three real Garage Tips articles, published and ready to view live. Run this whole script
-- once in the Supabase SQL Editor (Project > SQL Editor > New query).
--
-- Each article is a self-contained DO block that UPDATEs the article if a row with that title
-- already exists for your account (e.g. you ran an earlier version of this script), or INSERTs
-- it if it doesn't. That means this script is safe to run more than once, and safe to run even
-- if you already ran a previous draft of these three articles -- it will just overwrite the old
-- content with this version rather than skipping or duplicating.
--
-- Images: most entries use real, hotlinked product photos (manufacturer/retailer CDNs, plus two
-- Wikimedia Commons photos that are freely licensed -- one CC BY-SA 4.0 with credit given inline,
-- one US-government public domain). Five of the ten table saws (SawStop ICS, both Grizzlys, the
-- Delta, and the RIDGID) come from sites that block hotlinking outright -- confirmed by repeated
-- 403 responses straight from their own servers, across their own site and multiple resellers.
-- Those five use a small original line-art icon instead of a broken image; their manufacturer
-- links still work fine for a normal click, since that's a real browser navigation, not a
-- hotlinked asset request.
...
begin
    ...
    set excerpt = v_excerpt, body_html = v_body, category = 'Other',
        ...
        published = true, author = 'Kerph Team', updated_at = now()
    where user_id = v_user_id and title = v_title;

    get diagnostics v_updated = row_count;

    if v_updated = 0 then
        insert into public.garage_tips (user_id, title, excerpt, body_html, category, tags, published, author)
...
declare
    ...
    v_excerpt text := $excerpt$Nobody skips maintenance on purpose -- it happens because there's no rhythm to it. A weekly, monthly, and quarterly routine for the three stationary tools doing the most work in a typical shop.$excerpt$;
    v_body text := $body$
...
begin
    ...
    set excerpt = v_excerpt, body_html = v_body, category = 'Tool Maintenance',
        tags = array['maintenance','table-saw','band-saw','jointer','planer','tool-care'],
        published = true, author = 'Kerph Team', updated_at = now()
    where user_id = v_user_id and title = v_title;
    ...
    if v_updated = 0 then
        ...
        values (v_user_id, v_title, v_excerpt, v_body, 'Tool Maintenance',
                array['maintenance','table-saw','band-saw','jointer','planer','tool-care'], true, 'Kerph Team');
    end if;
end $$;
...
declare
    ...
    v_excerpt text := $excerpt$Dust collection fails quietly. Sizing, layout, and the small mistakes that quietly rob a dust collector of half its suction -- a practical list for planning or fixing your shop's dust collection.$excerpt$;
    v_body text := $body$
...
begin
    ...
    update public.garage_tips
    set excerpt = v_excerpt, body_html = v_body, category = 'Dust Collection',
        tags = array['dust-collection','shop-setup','airflow','safety'],
        published = true, author = 'Kerph Team', updated_at = now()
    where user_id = v_user_id and title = v_title;
    ...
    if v_updated = 0 then
        insert into public.garage_tips (user_id, title, excerpt, body_html, category, tags, published, author)
        values (v_user_id, v_title, v_excerpt, v_body, 'Dust Collection',
                array['dust-collection','shop-setup','airflow','safety'], true, 'Kerph Team');
    end if;
end $$;
